...
public sealed class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        foreach (var queueName in RabbitMqTopology.DirectQueueBindings.Keys)
        {
            ...
            consumer.ReceivedAsync += async (_, eventArgs) =>
            {
                ...
                _logger.LogInformation(
                    "Direct worker received message from queue {QueueName}: {@Message}",
                    queueName,
                    message);
            ...
            _logger.LogInformation(
                "Direct worker is listening to queue {QueueName}",
                queueName);
        }
